...
const Home = () => {
  ...
  // Unified helper to get accurate balance and status
  const getInvoiceMetrics = (inv) => {
  ...
  const getSmartStats = () => {
    ...
    if (!invoices) return initialAcc;

    return invoices.reduce((acc, inv) => {
      const { status, balance } = getInvoiceMetrics(inv);
      ...
      if (balance > 0) acc.unpaid.push(inv);
  ...
  const totalRevenue =
    invoices?.reduce((acc, inv) => acc + (inv.totalAmount || 0), 0) || 0;

  // Filter logic based on the unified metrics
  const filteredInvoices = invoices?.filter((inv) => {
    const { status, balance } = getInvoiceMetrics(inv);
    if (activeFilter === "All") return true;
    if (activeFilter === "Unpaid") return balance > 0;
  ...
  const stats = [
    {
      label: "Overdue",
      value: smartData.overdue.length,
    ...
    {
      label: "Completed",
      value: smartData.completed.length,
      icon: FaCheckCircle,
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm font-bold uppercase tracking-widest opacity-70">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((stat, i) => (
            ...
            >
              <div
                className={`${stat.bg} ${stat.color} w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-xl shadow-inner`}
              >
                <stat.icon />
              </div>
              <p className="text-[10px] uppercase text-slate-400 font-black tracking-[2px]">
                {stat.label}
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white border-2 border-slate-100 rounded-[2rem] shadow-sm overflow-hidden flex flex-col h-full">
              {/* HEADER SECTION */}
              ...
              {/* SCROLLABLE TABLE CONTAINER - Logic: h-[450px] if > 7 records, else h-auto */}
              <div
                className={`overflow-x-auto overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 ${
              ...
              >
                <table className="w-full text-left border-collapse relative">
                  {/* STICKY THEAD - Stays visible while scrolling */}
                  <thead className="bg-slate-50/90 backdrop-blur-sm text-[10px] font-black uppercase text-slate-400 tracking-widest sticky top-0 z-10 border-b border-slate-100">
                    <tr>
                        ...
                        status === "Overdue"
                          ...
                          : status === "Urgent"
                            ...
                            <td className="px-8 py-5 text-right">
                              <p
                                className={`font-black ${balance > 0 ? "text-rose-600" : "text-emerald-500"}`}
                              >
                                {balance > 0
                      ...
                      <tr>
                        <td colSpan="4" className="text-center py-32">
                          <div className="flex flex-col items-center opacity-20">
                            ...
                            <p className="font-black uppercase tracking-[4px] text-xs">
              ...
              {filteredInvoices?.length > 0 && (
                <div className="px-8 py-4 border-t border-slate-100 bg-slate-50/20">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Total Entries: {filteredInvoices.length}
                  </p>
                </div>
              )}
            </div>
